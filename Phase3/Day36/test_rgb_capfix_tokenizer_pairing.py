"""
實測驗證:best_model_rgb_full_capfix_reweight2x.pt 該配哪份 tokenizer。
"""
import sys
...
CKPT_PATH = DAY32 / "checkpoints" / "best_model_rgb_full_capfix_reweight2x.pt"
...
CANDIDATES = {
    "tokenizer_rgb_full_capfix.pkl (重建, Day32)": DAY32 / "tokenizer_rgb_full_capfix.pkl",
    "tokenizer_rgb_full.pkl (rgb舊版, Day32)": DAY32 / "tokenizer_rgb_full.pkl",
...
def main():
    ...
    for cand_name, cand_path in CANDIDATES.items():
        ...
        for fn, feat in zip(TEST_IMAGES, img_feats):
            torch.manual_seed(1337)

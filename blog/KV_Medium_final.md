...
![alt text](images/eq_kv_definition.png)

- `x_i = tok_emb(t_i) + pos_emb(i)`，是位置 `i` 上 token `t_i` 的輸入向量
- `W_k, W_v` 是投影矩陣，**訓練完成後就凍結，推論時完全不變**
- `tok_emb, pos_emb` 是查表運算，輸入 token id / 位置編號，永遠回傳同一個向量
- `LayerNorm` 只依賴 `x_i` 自己（**per-token 獨立、不跨 token 混合**）

由上述幾點可以得知，只要 token 還在同一個位置，不管在哪一步生成也不管後面接了什麼資料，
`k_i` 和 `v_i` 的值一定都會一樣。那當我們在算最新的第 `N` 個 token 時，
`k_0, k_1, ..., k_(N-1)` 和 `v_0, v_1, ..., v_(N-1)` 就會是已知的。那解法就也很直觀了，透過將已知的 `k` 和 `v` 進行 cache 使得避免之後的重複運算，來達成加速的效果，這也就是 KV cache 這名字的由來。

實際上的做法也很簡單，生成兩個裝入 `K` 和 `V` 的 cache，每次計算新的 token `(N)` 時，就把 `V_N` 和 `K_N` 接在 cache 的尾端，這樣就可以取得完整的 `k_0, k_1, ..., k_N` 和 `v_0, v_1, ..., v_N` 來用於 Attention 的計算。序列越長，加速的效果就會越明顯。
...
![alt text](images/table_benchmark_result.png)
...
![alt text](images/table_t_comparison.png)
...
![alt text](images/table_memcpy_source.png)
...
![alt text](images/table_prefix_postfix.png)
